...
//  Fuchen
//
//  Wrapper around the `mo` command. Fuchen doesn't ship Mole — it depends
//  on a system-installed copy (`brew install mole`), found via PATH.
//
//  Three commands matter to Fuchen today:
...
enum MoleCLI {
    ...
    /// doesn't include Homebrew's bin directory. Result is cached for the
    /// process lifetime.
    static func findExecutable() -> String? {
        if let cachedExecutable { return cachedExecutable }
        let found = locateExecutable()
    ...
    private static func locateExecutable() -> String? {
        ...
        // Last resort: ask the shell. Will work if the user launched Fuchen
    ...
    /// Modal alert shown at launch when `mo` isn't installed.
