...
enum L10n {
    ...
    static var waitingHint: String {
        t("拂尘会定时运行 `mo status --json`，首次数据将在一个采样周期内到达。",
          en: "Fuchen runs `mo status --json` on a timer; the first row lands within a tick.")
    }
    ...
    static var cleanCachesBody: String {
        t("拂尘将以管理员权限运行 `mo clean`。缓存文件将被永久删除；Mole 的白名单与安全规则仍然生效。",
          en: "Fuchen will run `mo clean` with administrator rights. Cache files are removed permanently; Mole's whitelist and safety rules still apply.")
    }
    ...
    static var refreshSizesHint: String { t("点击 ↻ 计算应用大小（本地快速统计，不跑 mole）", en: "Click ↻ to compute sizes locally (no mole subprocess)") }
    ...
    static var samplingFootnote: String {
        t("拂尘按此间隔运行 `mo status --json`。60 秒对图表已足够；更短间隔细节更细，但子进程开销更大。",
          en: "Fuchen runs `mo status --json` at this cadence. 60 s is plenty for charts; tighter intervals give finer detail at the cost of more subprocess churn.")
    }
    ...
    static var moleNotFoundBody: String {
        t("""
        拂尘依赖 Mole CLI（`mo`）获取系统指标与清理能力。请先安装：
        ...
        """,
        en: """
        Fuchen uses the Mole CLI (`mo`) for system metrics and cleanup. \
        Install it with:
