...
//  but with Fuchen's own palette and our own taglines (no planets, no
//  borrowed copy). `navOrder` is the left-to-right order in the top
//  pill nav; `.status` is where Fuchen opens because the live dashboard
...
/// Everything the main window can show. The five tools plus Fuchen's two
